#' Function to calculate the gradient of Ufunc (for both X and Y coordinates).
#'
...
#function to concatenated vector of dU/dXs and dU/dYs for concatenated vector of Xs and Ys
dUfunc<-function(XY,gam2,Freq){
	#Create FiFj matrix
	FiFj<-Freq%*%t(Freq)
	#Create X and Y
	nv<-length(XY)/2
	...
	#Now du/dX and du/dy vectors
	dudx<-rowSums(FiFj*6*(2*gam2-1)*YimYj*Umat*(1-Umat))
	dudy<-rowSums(FiFj*6*(2*gam2-1)*XimXj*Umat*(1-Umat))
	return(c(dudx,dudy))
	}